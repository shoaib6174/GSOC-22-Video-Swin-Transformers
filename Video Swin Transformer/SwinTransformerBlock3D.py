...
class SwinTransformerBlock3D(tf.keras.layers.Layer):
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        if any(i > 0 for i in shift_size):
            shifted_x = tf.roll(x, shifts=[shift_size[0], -shift_size[1], -shift_size[2]], dims=[1, 2, 3]) #?
        ...
        if any(i > 0 for i in shift_size):
            x = tf.roll(shifted_x, shifts=[shift_size[0], shift_size[1], shift_size[2]], dims=[1, 2, 3]) #?
        else:
            x = shifted_x
